...
# Replace dimshuffle from https://stackoverflow.com/questions/35163789/theano-dimshuffle-equivalent-in-googles-tensorflow
...
# MMD from https://github.com/wzell/mann/blob/master/models/maximum_mean_discrepancy.py
def mmd(x1, x2, beta = 0.1):
    ...
    """
    x1x1 = breit_wigner_kernel(x1, x1, beta)
    x1x2 = breit_wigner_kernel(x1, x2, beta)
    x2x2 = breit_wigner_kernel(x2, x2, beta)
    diff = K.mean(x1x1) - 2 * K.mean(x1x2) + K.mean(x2x2)
    return diff

def gaussian_kernel(x1, x2, beta = 1.0):
    r = dimshuffle(x1, [0, 1],'x')
    return K.sum(K.exp( -beta * K.square(r - x2)),axis=-1)

def breit_wigner_kernel(x1, x2, beta = 1.0):
    beta_sq = beta*beta
    return beta_sq/(K.square(x1 - x2) + beta_sq)
...
class WGANGP():
    ...
    def gradient_penalty_loss(self, y_true, y_pred, averaged_samples):
        ...
        return K.mean(gradient_penalty) + mmd(y_pred, y_true)
    ...
    def build_critic(self, units=128, alpha=0.2, dropout=0.25):
        ...
        init = RandomNormal(stddev=0.02)
        for i in range(3):
            model.add(Dense(units, kernel_initializer=init, input_shape=(26,)))
            model.add(LeakyReLU(alpha=alpha))
...
if __name__ == '__main__':
    ...
    for iteration in tqdm(range(INTERMEDIATE_STEPS)):
        gan.train(epochs=int(TOTAL_EPOCHS/INTERMEDIATE_STEPS), batch_size=1024)
        ...
        np.savetxt('wgangpmbd_events_32ksteps_100lat-MMD-breitwigner'+str(iteration)+'.csv', gen_events, delimiter=' ') 
