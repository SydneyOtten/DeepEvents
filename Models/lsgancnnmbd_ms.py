...
class LSGAN():
	...
	def build_generator(self):
		...
		model.add(Dense(128, kernel_initializer='glorot_uniform', input_dim=self.latent_dim))
		#G = Dropout(0.2)(G)
		model.add(LeakyReLU(alpha=0.2))
		#G = Activation("relu")(G)
		model.add(BatchNormalization())

		model.add(Reshape([8, 8, 2])) # default: channel last
		
		model.add(Conv2DTranspose(16, kernel_size=2, strides=1, padding="same"))
		#G = Activation("relu")(G)
		model.add(LeakyReLU(alpha=0.2))
		model.add(BatchNormalization())
	...
	def build_discriminator(self):
		...
		model.add(MinibatchDiscrimination(8,16))
		model.add(Dense(1))
	...
	def train(self, epochs, batch_size=128, sample_interval=50):
		...
		for epoch in range(epochs):
			...
			gen_imgs = np.reshape(gen_imgs, (-1,26,1))
...
if __name__ == '__main__':
	...
	one_d_hist = []
	two_d_hist = []
	gan.train(epochs=500000, batch_size=1000, sample_interval=200)
	n_samples = 1200000
	noise = np.random.normal(0, 1, (n_samples, 500))
	gen_events = gan.generator.predict(noise)
	gen_events = np.reshape(gen_events, (n_samples, 26))
	np.savetxt('ls_gan_events_192ksteps.csv', gen_events, delimiter=' ') 
